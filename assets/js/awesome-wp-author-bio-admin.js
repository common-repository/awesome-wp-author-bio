(function($) {
    ...
    // Notice Hide
    $('body').on('click', '.jltauthorbio-upgrade-popup .popup-dismiss', function(evt) {
        evt.preventDefault();
        $(this).closest('.jltauthorbio-upgrade-popup').fadeOut(200);
    });

    // Notice Show
    $('body').on('click', '.disabled', function(evt) {
        evt.preventDefault();
        $('.jltauthorbio-upgrade-popup').fadeIn(200);
    });
    ...
    // Search
    $('.jltauthorbio-search-plugins #search-plugins').on('keyup', function() {
        var value = $(this).val();
...
function activatePlugin(element, file) {
    ...
    jQuery.ajax({
        url: JLTAUTHORBIOCORE.admin_ajax,
        type: "POST",
        data: {
            action: "jltauthorbio_recommended_activate_plugin",
            file: file,
            nonce: JLTAUTHORBIOCORE.recommended_nonce
        },
...
function installPlugin(element, plugin) {
    ...
    jQuery.ajax({
        ...
        type: "POST",
        data: {
            action: "jltauthorbio_recommended_upgrade_plugin",
            type: 'install',
            plugin: plugin,
            nonce: JLTAUTHORBIOCORE.recommended_nonce
        },
