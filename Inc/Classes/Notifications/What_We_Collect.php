...
if ( ! class_exists( 'What_We_Collect' ) ) {
	...
	class What_We_Collect extends Notice {
		...
		public function __construct() {
			parent::__construct();
			add_action( 'wp_ajax_jltauthorbio_allow_collect', array( $this, 'jltauthorbio_allow_collect' ) );
		}
		 ...
		 * @author Jewel Theme <support@jeweltheme.com>
		 */
		public function jltauthorbio_allow_collect() {
			check_ajax_referer( 'jltauthorbio_allow_collect_nonce' );
		...
		public function what_we_collect_link() {
			echo '<a href="!#" class="jltauthorbio-wwc-link">' . esc_html__( 'what we collect', 'awesome-wp-author-bio' ) . '</a>';
		}
		...
		public function notice_content() { ?>
			<div class="jltauthorbio-notice-review-box jltauthorbio-wwc">
				<p>
				<?php
				echo sprintf(
					__( 'Want to help make <strong>%1$s</strong> even more awesome? Allow %1$s to collect non-sensitive diagnostic data and usage information.', 'awesome-wp-author-bio' ),
					__( 'awesome-wp-author-bio', 'awesome-wp-author-bio' )
				);
				?>
				 (<?php $this->what_we_collect_link(); ?>)</p>
				<div class="jltauthorbio-wwc-content" style="display:none">
					<?php echo sprintf(
						__( 'Server environment details (php, mysql, server, WordPress versions), Number of users in your site, Site language, Number of active and inactive plugins, Local or Production Site, IP Address, Site name and url, Your name and email address etc. No sensitive data is tracked. Learn more about our <a href="%1$s" target="_blank">Privacy Policy</a>, how we handle and collects your data.', 'awesome-wp-author-bio' ),
		...
		public function footer_content() {
			...
			<a class="button button-primary allow-button" href="<?php echo esc_url( $this->plugin_rate_url() ); ?>" rel="nofollow" target="_blank">
				<?php echo esc_html__( 'Allow', 'awesome-wp-author-bio' ); ?>
			</a>
			<a class="button button-secondary button-reject jltauthorbio-notice-dismiss" href="#" rel="nofollow">
				<?php echo esc_html__( 'No Thanks', 'awesome-wp-author-bio' ); ?>
			</a>
			<?php
		}
		...
		public function core_script( $trigger_time ) {
			...
			<script>
				jQuery('body').on('click', '.jltauthorbio-notice-what_we_collect .jltauthorbio-wwc-link', function(e) {
					e.preventDefault();
					if (jQuery(this).hasClass('show')) {
						jQuery(this).removeClass("show");
						jQuery('.jltauthorbio-wwc-content').stop().slideUp(200);
					} else {
						jQuery(this).addClass("show");
						jQuery('.jltauthorbio-wwc-content').stop().slideDown(200);
					}
				});

				jQuery('body').on('click', '.jltauthorbio-notice-what_we_collect .allow-button', function(e) {

					e.preventDefault();

					let noticeWrapper = jQuery(this).closest('.notice-jltauthorbio');
					...
					jQuery.ajax({
							...
							crossDomain: true,
							data: {
								action: 'jltauthorbio_allow_collect',
								_wpnonce: '<?php echo esc_js( wp_create_nonce( 'jltauthorbio_allow_collect_nonce' ) ); ?>',
							}
						})
						.done(function(response) {
							noticeWrapper.children(':not(.notice-dismiss)').hide().end().append('<p class="jltauthorbio--notice-message"><strong>' + response.data + '</strong></p>');
							let subsTimeout = setTimeout(function() {
								jltauthorbio_notice_action(null, noticeWrapper.children(), 'disable');
