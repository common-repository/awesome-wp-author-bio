...
class Pro_Upgrade {
	...
	public function dashboard_widget() {
		wp_add_dashboard_widget(
			'jltauthorbio_dashboard_widget',                          // Widget slug.
			esc_html__( 'awesome-wp-author-bio News & Updates', 'awesome-wp-author-bio' ), // Title.
		...
		$example_widget_backup = array( 'jltauthorbio_dashboard_widget' => $default_dashboard['jltauthorbio_dashboard_widget'] );
		unset( $default_dashboard['jltauthorbio_dashboard_widget'] );
	...
	public function dashboard_widget_render() {
		...
		echo '<div class="jltauthorbio-rss-widget">';

		if ( wp_validate_boolean( $this->get_content( 'is_campaign' ) ) ) { ?>

			<div class="jltauthorbio-dashboard-promo" style="--jltauthorbio-popup-color: <?php echo esc_attr( $this->get_content( 'btn_color' ) ); ?>;">
				<a target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>">
					<img src="<?php echo esc_url( $this->get_content( 'image_url' ) ); ?>" alt="awesome-wp-author-bio Promo Image" style="width: 100%; height: auto;">
				</a>

				<a class="jltauthorbio-popup-button" target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>">
		...
		if ( ! empty( $maxitems ) ) {
			...
			foreach ( $rss_items as $item ) {
				...
				$item_date = human_time_diff( $item->get_date( 'U' ), current_time( 'timestamp' ) ) . ' ' . __( 'ago', 'awesome-wp-author-bio' );
		...
		<div class="jltauthorbio-dashboard_footer">
			<ul>
				<li class="jltauthorbio-overview__blog">
                    <a href="https://jeweltheme.com/blog" target="_blank">
                        Blog
                        <span class="screen-reader-text">
                            <?php echo esc_html__( '(opens in a new window)', 'awesome-wp-author-bio' ); ?>
                        </span>
                        <span aria-hidden="true" class="dashicons dashicons-external"></span>
                    </a>
                </li>
                <li class="jltauthorbio-overview__help">
                ...
                <li class="jltauthorbio-overview__upgrade">
		...
		<style>
			...
			.jltauthorbio-rss-widget .hndle.ui-sortable-handle img {
				margin: -5px 10px -5px 0;
			}

			.jltauthorbio-rss-widget .jltauthorbio-dashboard_footer {
				margin: 0 -12px -12px;
				padding: 0 12px;
				border-top: 1px solid #eee;
			}

			.jltauthorbio-rss-widget .jltauthorbio-dashboard_footer ul {
				display: flex;
				list-style: none;
			}

			.jltauthorbio-rss-widget .jltauthorbio-dashboard_footer ul li:first-child {
				padding-left: 0;
				border: none;
			}

			.jltauthorbio-rss-widget .jltauthorbio-dashboard_footer li {
				padding: 0 10px;
				margin: 0;
				border-left: 1px solid #ddd;
			}

			.jltauthorbio-rss-widget .jltauthorbio-overview__go-pro a {
				color: #FCB92C;
				font-weight: 500;
			}
		</style>
	...
	public static function get_data() {
		return get_option( 'jltauthorbio_sheet_promo_data' );
	}
	...
	public function get_data_hash() {
		return get_option( 'jltauthorbio_sheet_promo_data_hash' );
	}
	...
	public function register_sync_hook() {
		$hook_action = 'jltauthorbio_sheet_promo_data_remote_sync';
		...
		register_deactivation_hook( JLTAUTHORBIO_FILE, array( $this, 'clear_register_sync_hook' ) );
	}
	...
	public function clear_register_sync_hook() {
		wp_clear_scheduled_hook( 'jltauthorbio_sheet_promo_data_remote_sync' );
	}
	...
	public function sheet_data_remote_sync() {
		...
		if ( $force || $sheet_hash_data !== $sheet_data_hash ) {
			update_option( 'jltauthorbio_sheet_promo_data', $remote_data );
			update_option( 'jltauthorbio_sheet_promo_data_hash', $sheet_data_hash );
			do_action( 'jltauthorbio_sheet_promo_data_reset' );
		}
	}
	...
	public function get_sheet_promo_remote_data() {
		...
		$plugin_slug = Helper::jltauthorbio_slug_cleanup();
	...
	public function display_popup() {
		...
		<div class="jltauthorbio-popup jltauthorbio-upgrade-popup" id="jltauthorbio-popup" data-plugin="<?php echo esc_attr( $this->slug ); ?>" tabindex="1" style="display: none;">

			<div class="jltauthorbio-popup-overlay"></div>

			<div class="jltauthorbio-popup-modal" style="background-image: url('<?php echo esc_url( $image_url ); ?>'); --jltauthorbio-popup-color: <?php echo esc_attr( $this->get_content( 'btn_color' ) ); ?>;">
				...
				<div class="jltauthorbio-popup-modal-footer">
					...
					<div class="jltauthorbio-popup-countdown" style="display: none;">
						<span class="jltauthorbio-popup-countdown-text">Deal Ends In</span>
					...
					<!-- button  -->
					<a class="jltauthorbio-popup-button" target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>"><?php echo esc_html( $this->get_content( 'button_text' ) ); ?></a>
				</div>
			</div>
		</div>

		<script>
			var $container = jQuery('#jltauthorbio-popup'),
				plugin_data = <?php echo json_encode( $this->get_sheet_promo_remote_data(), true ); ?>,
				events = {}; //Events

			// Update Counter
			function updateCounter(seconds) {
				const $counter = $container.find(".jltauthorbio-popup-countdown-time");
			...
			function initCounter(last_date) {
				$container.find(".jltauthorbio-popup-countdown-time").show();
				...
				if (result) {
					...
					$container.find(".jltauthorbio-popup-countdown").show(0);
				} else {
					trigger("countdownFinish", [plugin_data]);
					$container.find(".jltauthorbio-popup-countdown").hide(0);
				}
